...
public class Quantity<U> where U : Enum
{
    ...
    public Quantity<U> Subtract(Quantity<U> other, U targetUnit)
    {
        ...
        double resultBase = ToBaseUnit() - other.ToBaseUnit();

        double result;

        if (targetUnit is LengthUnit lu)
            result = lu.ConvertFromBaseUnit(resultBase);

        else if (targetUnit is WeightUnit wu)
            result = wu.ConvertFromBaseUnit(resultBase);

        else if (targetUnit is VolumeUnit vu)
            result = vu.ConvertFromBaseUnit(resultBase);
